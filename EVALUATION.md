...
| `prana-model` | engine-side model loading, tokenizer, sampling | **runs a real trained model.** Loads llama2.c checkpoints *and GGUF v2/v3* for **llama, qwen2, gemma** architectures (F32/F16/Q8_0/**Q4_K/Q6_K**; every quantized type runs natively in packed form — Q8_0 as `QuantMatrix`, K-quants on dedicated packed-block matmuls at 4.5/6.6 bits per weight, parity-tested against dequantized dense). SentencePiece and GPT-2 byte-level BPE tokenizers; per-arch knobs (QKV biases, NeoX vs interleaved RoPE, GELU vs SiLU, (1+w) norm folding, √dim embedding scale, decoupled head_dim) exercised by synthetic-GGUF fixtures. Greedy llama output is token-identical to llama2.c's reference generation via both container formats. Zero `unsafe`, zero dependencies |
