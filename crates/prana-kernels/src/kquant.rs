//! K-quant (Q4_K / Q6_K) block formats: dequantization *and* native matmul.
//!
//! These are llama.cpp's super-block formats — 256 weights per block with
//! hierarchical scales — and the dominant types inside `Q4_K_M` model files.
//! Storing the packed blocks and dotting against them directly keeps weights
//! at ~4.5 / ~6.6 bits each in RAM (vs 32 after dequantize-at-load) and reads
//! 7x less weight memory per token, which is what decode is bound by.
//!
...
/// A row-major matrix stored as packed K-quant super-blocks.
pub struct KQuantMatrix {
    pub rows: usize,
    pub cols: usize,
    kind: KQuantKind,
    /// `rows * cols/256 * block_bytes`, rows contiguous.
    blocks: Vec<u8>,
}
...
impl KQuantMatrix {
    ...
    pub fn from_raw(rows: usize, cols: usize, kind: KQuantKind, blocks: Vec<u8>) -> Self {
        assert_eq!(cols % QK_K, 0, "cols must be a multiple of {QK_K}");
        assert_eq!(blocks.len(), rows * cols / QK_K * kind.block_bytes());
    ...
    fn row(&self, r: usize) -> &[u8] {
        let per_row = self.cols / QK_K * self.kind.block_bytes();
        &self.blocks[r * per_row..(r + 1) * per_row]
    }

    /// Full dequantization (tests / embedding-style access).
    pub fn dequantize(&self) -> Vec<f32> {
        let mut out = Vec::with_capacity(self.rows * self.cols);
        for b in self.blocks.chunks_exact(self.kind.block_bytes()) {
            match self.kind {
                KQuantKind::Q4K => dequant_q4k_block(b, &mut out),
                KQuantKind::Q6K => dequant_q6k_block(b, &mut out),
            }
        }
        out
    }
...
pub fn matmul_kquant_f32(a: &[f32], m: &KQuantMatrix, n_tokens: usize) -> Vec<f32> {
    ...
    for t in 0..n_tokens {
        ...
        let asums = match m.kind {
            KQuantKind::Q4K => group_sums(a_row),
...
mod tests {
    ...
    fn check_matches_dequant_reference(kind: KQuantKind, seed: u64) {
        ...
        let mut bytes = pseudo_bytes(rows * cols / QK_K * bb, seed);
